...
public class AgendarRideDiaSemanaService {
    ...
    public void desativar(Long userId, ListaDiaSemanas diasSemana) {

        User user = userRepository.findById(userId)
                .orElseThrow(() -> new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Usuario não encontrado"));

        Ride ride = rideRepository.findById(diasSemana.rideId()) // ✅ rideId vindo do DTO
                .orElseThrow(() -> new ResponseStatusException(
                        HttpStatus.NOT_FOUND, "Ride não encontrado"));

        // ✅ verifica que a ride pertence ao usuário autenticado
        ...
        List<AgendarRideDiaSemana> lista =
                agendarRideDiaSemanaRepository.findAllByRideId(diasSemana.rideId()); // ✅ rideId correto
