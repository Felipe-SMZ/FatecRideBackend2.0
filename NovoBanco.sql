...
create table estados(
  ...
  `ibge` BIGINT DEFAULT NULL,
  `pais` BIGINT DEFAULT NULL,
...
select * from cidade where id_estado = '26';
select count(*) from cidade;

-- Inserindo dados na tabela "cidade"
insert into cidade (id_cidade, nome, id_estado, ibge) VALUES
...
CREATE TABLE caronas (
    ...
    vagas_disponiveis INT NOT NULL,
	id_status_carona BIGINT NOT NULL,
	id_veiculo BIGINT NOT NULL,
    FOREIGN KEY (id_motorista) REFERENCES usuarios(id_usuario) ON DELETE CASCADE,
    FOREIGN KEY (id_origem) REFERENCES origens(id_origem) ON DELETE CASCADE,
    FOREIGN KEY (id_destino) REFERENCES destinos(id_destino) ON DELETE CASCADE,
	FOREIGN KEY (id_status_carona) REFERENCES status_carona(id_status_carona) ON DELETE CASCADE,
    FOREIGN KEY (id_veiculo) REFERENCES veiculos(id_veiculo)
);

CREATE TABLE solicitacoes (
    id_solicitacao BIGINT PRIMARY KEY AUTO_INCREMENT,
    id_carona BIGINT NOT NULL,
    id_passageiro BIGINT NOT NULL,
    id_origem BIGINT NOT NULL,
    id_destino BIGINT NOT NULL,
    data_solicitacao DATETIME DEFAULT CURRENT_TIMESTAMP,
	id_status_solicitacao BIGINT,
    FOREIGN KEY (id_carona) REFERENCES caronas(id_carona) ON DELETE CASCADE,
    FOREIGN KEY (id_passageiro) REFERENCES usuarios(id_usuario) ON DELETE CASCADE,
    FOREIGN KEY (id_origem) REFERENCES origens(id_origem) ON DELETE CASCADE,
    FOREIGN KEY (id_destino) REFERENCES destinos(id_destino) ON DELETE CASCADE,
	FOREIGN KEY (id_status_solicitacao) REFERENCES status_solicitacao(id_status_solicitacao)
);

-- Tabelas de histórico para usuários e veículos deletados
CREATE TABLE usuarios_deletados LIKE usuarios;
CREATE TABLE veiculos_deletados LIKE veiculos;
CREATE TABLE caronas_deletados LIKE caronas;
CREATE TABLE solicitacoes_deletados LIKE solicitacoes;
...
FOR EACH ROW
BEGIN
    DECLARE tipo_usuario_val VARCHAR(25);

    SELECT tipo INTO tipo_usuario_val
    FROM tipo_usuario
    WHERE id_tipo_usuario = OLD.id_tipo_usuario;

    INSERT INTO usuarios_deletados
    SELECT * FROM usuarios WHERE id_usuario = OLD.id_usuario;

    IF tipo_usuario_val = 'motorista' OR tipo_usuario_val = 'ambos' THEN
        INSERT INTO veiculos_deletados
        SELECT * FROM veiculos WHERE id_usuario = OLD.id_usuario;
		insert into caronas_deletados
			SELECT * FROM caronas WHERE id_motorista = OLD.id_usuario;
        
        DELETE FROM caronas WHERE id_motorista = OLD.id_usuario;
		DELETE FROM veiculos WHERE id_usuario = OLD.id_usuario;
...
-- Inserts (sem alteração, só para referência)
INSERT INTO curso (nome, description) VALUES 
('Análise e Desenvolvimento de Sistemas', 'Curso técnico de desenvolvimento de software'),
('Gestão da Tecnologia da Informação', 'Curso voltado para gestão de TI'),
('Logística', 'Curso voltado para logística e cadeia de suprimentos');
...
##update caronas set id_status_carona = 2 where id_carona  in (3,4,5,6,7);
...
o.logradouro as "origem logradouro", o.numero, o.bairro as "origem bairro", o.cep as "origem cep,", o.latitude as "origem latitude" , o.longitude as "origeem longitude",
d.logradouro as "destino logradouro" , d.numero as "destino numero" ,  d.bairro as "destino bairro", d.cep as "destino cep", d.latitude as "latitude latitude", d.longitude  as "destino longitude"
 from usuarios as u
 inner join  caronas as c
 on u.id_usuario = c.id_motorista
 inner join status_carona as status
 on c.id_status_carona = status.id_status_carona
  inner join veiculos as v
 on u.id_usuario = v.id_usuario
 inner join origens as o 
 on c.id_origem = o.id_origem
  inner join destinos as d
 on c.id_destino = d.id_destino
 where u.id_usuario = 3 AND status.status_nome = 'ativa';
...
status.status_nome,
v.modelo,v.marca, v.placa, v.cor, v.ano,
o.id_origem,o.logradouro as "origem logradouro", o.numero, o.bairro as "origem bairro", o.cep as "origem cep,", o.latitude as "origem latitude" , o.longitude as "origeem longitude",
d.id_destino, d.logradouro as "destino logradouro" , d.numero as "destino numero" ,  d.bairro as "destino bairro", d.cep as "destino cep", d.latitude as "latitude latitude", d.longitude  as "destino longitude"
 from usuarios as u
 inner join  caronas as c
 on u.id_usuario = c.id_motorista
 inner join status_carona as status
